...
pixel coordinates are in =.x= and =.y=, and the [[file:uncertainty.org::#noise model][observation weight]] is in =.z=.
...
The [[file:uncertainty.org::#noise model][expected uncertainty]] of the pixel observations is given in
