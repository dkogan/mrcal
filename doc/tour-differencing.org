...
extra image processing that's not modeled by =LENSMODEL_OPENV8=, so we see a
discrepancy in the center. The models agree well past that, and then the error
grows as we move towards the edges.
