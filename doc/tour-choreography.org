...
dance-study.py                   \
  ...
  --range 0.4,10                 \
  --observed-pixel-uncertainty 2 \
  --uncertainty-at-range-sampled-max 50          \
  opencv4.cameramodel
#+end_src
#+begin_src sh :exports none :eval no-export
D=~/projects/mrcal-doc-external/2022-11-05--dtla-overpass--samyang--alpha7/2-f22-infinity
...
dance-study.py                         \
  ...
  --observed-pixel-uncertainty 2       \
  --ymax 1                             \
  --uncertainty-at-range-sampled-max 4 \
  opencv4.cameramodel
#+end_src
#+begin_src sh :exports none :eval no-export
D=~/projects/mrcal-doc-external/2022-11-05--dtla-overpass--samyang--alpha7/2-f22-infinity
...
:PROPERTIES:
:CUSTOM_ID: choreography-Ncameras
:END:
...
:PROPERTIES:
:CUSTOM_ID: choreography-chessboard-density
:END:
...
:PROPERTIES:
:CUSTOM_ID: choreography-chessboard-size
:END:
...
dance-study.py                                     \
  ...
  --object-spacing 0.04,0.20                       \
  --ymax 6                                         \
  --uncertainty-at-range-sampled-max 60            \
  --observed-pixel-uncertainty 2                   \
  opencv4.cameramodel
#+end_src
#+begin_src sh :exports none :eval no-export
...
particular scenario using a =LENSMODEL_OPENCV4= lens, and produce clear rules of
