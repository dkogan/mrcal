...
operation, with lots of diagnostics. This is very useful on its own, or to debug
a misbehaving ranging system.

Let's use the Downtown Los Angeles images in rthe [[file:tour-stereo.org][the tour of mrcal]]. Before we
start, one important caveat: there's only one camera, which was calibrated
monocularly. The one camera was moved to capture the two images used to
triangulate. The extrinsics were computed with a not-yet-in-mrcal tool, and
mrcal cannot yet propagate the calibration noise in this scenario. Thus in this
example we only propagate the observation-time noise.
