...
However, when running the triangulation routines with near-parallel rays, his
assumptions can break down.

Let's run another simulation, but observing a more distant point, with more
noise and gathering more samples:
...
# @@ -954,7 +954,7 @@ if args.make_documentation_plots is not None:
#                  **processoptions)
# 
