...
More complex usages are available via Python APIs (or the C functions in
...
1. For successful long-range stereo we need maximum precision in our lens
   ...
   is great, but today, a calibration based on this model cannot be used in any
   tool other than mrcal.

2. Furthermore, mrcal can use a [[#stereo-rectification-models][=LENSMODEL_LATLON=]] to describe the rectified
   system instead of the more traditional pinhole rectification function. This
   allows nice stereo matching even with wide lenses, but once again: these
   rectified models and images can only be processed with mrcal.
