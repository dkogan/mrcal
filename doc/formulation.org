...
\xymatrix @C=5em {
\vec q &
\vec p_\mathrm{camera}   \ar[l]^{\vec b_\mathrm{intrinsics} } &
\vec p_\mathrm{reference}\ar[l]^{T_\mathrm{cr}} &
\vec p_\mathrm{frame}    \ar[l]^{T_\mathrm{rf}}
}
\]
...
I'm trying to find a relationship between the solve residuals $\vec x^*$ (which
I have from the solve) and the input noise $\Delta \vec q_\mathrm{ref}$. Let's
start with a perfect noise-free solve: the observations $\vec q_\mathrm{ref}$
are perfect and $\vec x = \vec 0$. Then I add some input noise $\Delta \vec
q_\mathrm{ref}$. I now have

\[\vec x_\mathrm{observations} = -W \Delta \vec q_\mathrm{ref}\]
...
\[
\Delta \vec b = \left( J^T J \right)^{-1} J_\mathrm{observations}^T W \Delta \vec q_\mathrm{ref}
\]

Let's extend $W$ to $W_\mathrm{full} = \left( \begin{matrix} W \\ 0 \end{matrix}
\right)$ to include the regularization measurements with weight 0. So

\[
\Delta \vec b = \left( J^T J \right)^{-1} J^T W_\mathrm{full} \Delta \vec q_\mathrm{ref}
\]
...
\begin{aligned}
\vec x^* &= -W_\mathrm{full} \Delta \vec q_\mathrm{ref} + \Delta \vec x \\
&= -W_\mathrm{full} \Delta \vec q_\mathrm{ref} + J \Delta \vec b \\
&= -W_\mathrm{full} \Delta \vec q_\mathrm{ref} + J \left( J^T J \right)^{-1} J^T W_\mathrm{full} \Delta \vec q_\mathrm{ref} \\
&= \left( -I + J \left( J^T J \right)^{-1} J^T \right) W_\mathrm{full} \Delta \vec q_\mathrm{ref}
\end{aligned}
...
\Delta \vec q_\mathrm{ref}^T W_\mathrm{full}^T \left( I - J \left( J^T J \right)^{-1} J^T \right) W_\mathrm{full} \Delta \vec q_\mathrm{ref} \\
&= \Delta \vec q_\mathrm{ref}^T W_\mathrm{full}^T M W_\mathrm{full} \Delta \vec q_\mathrm{ref} \\
\end{aligned}

As noted [[#noise-in-measurement-vector][below]], $W \Delta \vec q_\mathrm{ref}$ is 0-mean, and $\mathrm{Var}\left(W \Delta \vec q_\mathrm{ref}\right) = \sigma^2 I$. So
$W_\mathrm{full} \Delta \vec q_\mathrm{ref}$ is also 0-mean, and
$\mathrm{Var}\left(W_\mathrm{full} \Delta \vec q_\mathrm{ref}\right) = \sigma^2
\mathrm{diag}\left(I_\mathrm{observations} \; 0\right)$. So

\[
E\left( \left\Vert \vec x \right\Vert^2 \right) &= \mathrm{tr} \left( M_\mathrm{observations} \right) \sigma^2
\]

Where $M_\mathrm{observations}$ is the subset of $M$ corresponding /only/ to the
observations measurements, /not/ the regularization. I get

\begin{aligned}
E\left( \left\Vert \vec x \right\Vert^2 \right) &= \mathrm{tr} \left( M_\mathrm{observations} \right) \sigma^2 \\
&= \mathrm{tr} \left( I_\mathrm{observations} - J_\mathrm{observations} \left( J^T J \right)^{-1} J_\mathrm{observations}^T \right) \sigma^2 \\
&= \mathrm{tr} \left( I_\mathrm{observations} - J \left( J^T J \right)^{-1} J^T + J_\mathrm{regularization} \left( J^T J \right)^{-1} J_\mathrm{regularization}^T \right) \sigma^2 \\
&=
\left(N_{\mathrm{measurements}_\mathrm{obs}} - N_\mathrm{state} + \mathrm{tr}\left(J_\mathrm{regularization} \left( J^T J \right)^{-1} J_\mathrm{regularization}^T \right) \right) \sigma^2
\end{aligned}

The regularization terms are meant to be insignificant, and should move little
in response to state changes, so I ignore them here:

\[
E\left( \left\Vert \vec x \right\Vert^2 \right) \approx
\left(N_{\mathrm{measurements}_\mathrm{obs}} - N_\mathrm{state} \right) \sigma^2
\]

Here $N_{\mathrm{measurements}_\mathrm{obs}}$ is the number of measurements in $\vec x$ from
pixel observations, /not/ from regularization.

So the RMS reprojection error in a solve can be estimated as

\[
\sqrt{ \frac{E\left( \left\Vert \vec x \right\Vert^2 \right)}{N_{\mathrm{measurements}_\mathrm{obs}}}} =
\sqrt{ \frac{\left(N_{\mathrm{measurements}_\mathrm{obs}} - N_\mathrm{state}\right) \sigma^2}{N_{\mathrm{measurements}_\mathrm{obs}}}} =
\sigma \sqrt{1 - \frac{N_\mathrm{state}}{N_{\mathrm{measurements}_\mathrm{obs}}}}
\]

If we have information-full data, such as chessboard observations, then
$N_{\mathrm{measurements}_\mathrm{obs}} >> N_\mathrm{state}$, and we can simply
assume that the standard deviation of the input observation noise $\sigma$ is
the RMS reprojection error in the solve. However, with lower-quality data, such
as point observations, the extra correction factor of $\sqrt{1 -
\frac{N_\mathrm{state}}{N_{\mathrm{measurements}_\mathrm{obs}}}}$ cannot be
ignored.

Today this is implemented in [[file:mrcal-python-api-reference.html#-_observed_pixel_uncertainty_from_inputs][=mrcal._observed_pixel_uncertainty_from_inputs()=]].
