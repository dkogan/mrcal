...
/central/. This means that all rays of light are assumed to intersect at a
single point: the origin of the camera coordinate system. So $k \vec v$ projects
to the same $\vec q$ for any $k$. This is very convenient, but not completely
realistic. Support for /non-central/ lenses is coming in a future release of
mrcal.
