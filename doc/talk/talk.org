...
** Differencing
Let's compute the intrinsics difference using an obvious algorithm:
...
** Differencing
We can compensate for the errors by rotating the camera.
...
SAY THIS IS CRITICALLY IMPORTANT FOR SPLINED MODEL VALIDATION
...
b\right)$ to see how these shifts affect projection, and then
...
Let's fix it. We use a different flow to connect the baseline and perturbed
solves:
...
For any perturbation we explicitly solve for $T_\mathrm{rr^+}$ by optimizing the
cross path in red.
...
#+ATTR_LATEX: :width 0.9\textwidth :height 0.7\textheight :options keepaspectratio
[[file:../out/external/figures/moving-camera.pdf]]

This has the same issues as the mean-pcam uncertainty with far-off boards

Using the most-certain camera pose works, but it is slow

* Other ways of looking at this
We have been looking at diffs and uncertainty of /intrinsics, compensating for
extrinsics shifts/

There are other approaches, I've barely explored:

- With a /pair/ of cameras, we can do pairwise uncertainty and diffing.
  Maybe that's good-enough. This is joint intrinsics+extrinsics diff. Is that
  good-enough?

Uncertainty in point solves is already available in some conditions, for
instance if the points are fixed. New [[https://mrcal.secretsauce.net/mrcal-show-stereo-pair-diff.html][=mrcal-show-stereo-pair-diff= tool]] reports
an extrinsics+intrinsics diff between two calibrations of a stereo pair

experimental [[https://www.github.com/dkogan/mrcal/blob/master/analyses/extrinsics-stability.py][=analyses/extrinsics-stability.py=]] tool reports an extrinsics-only
diff.

validate-uncertainty does sampling. Is that right?

* might need a LOT of points
- dense-products libdogleg makes things fast
- also partially implemented: used for muse and era
