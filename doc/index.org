...
\[ \vec p_\mathrm{frame0} = T_\mathrm{f0r} T_\mathrm{rc} \mathrm{unproject}\left( \vec q \right) \]

This is a "fixed" point. I then transform and project $\vec p_\mathrm{frame0}$
back to the imager to get $q_1$. But here I take into account the uncertainties
of each transformation to get the desired projection uncertainty
$\mathrm{Var}\left(\vec q_1 - \vec q\right)$.

This works, but it depends on $\vec p_\mathrm{frame0}$ being "fixed", which
isn't entirely true. The frame coordinate systems are a part of the state
vector, so they move around 

How do I combine all the different estimates from the different chessboard
observations? I can take the mean of all the q1 I get from each frame to use
mean(q1)-q0 as the uncertainty. I will linearize the projection, so everything
is linear, and I will have
...
$T_\mathrm{ab}$ is a transformation of a point from coord system b to the same point
