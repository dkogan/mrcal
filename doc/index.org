...
And so on. The library is modular, and it is still possible to use traditional
opencv models and/or traditional dense stereo for those that need it. Using the
mrcal library to replace just one component of an existing pipeline is easy, and
a common way to utilize mrcal.
...
* Getting mrcal
mrcal is in Debian and Ubuntu, and for most people available with a single

#+begin_src sh
apt install mrcal
#+end_src

command. For more information, see the [[file:install.org]["installation, build instructions" page]].
