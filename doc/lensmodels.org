...
time of this writing these metadata keys are defined:
...
- =can_project_behind_camera=: True if this model is able to project vectors
  ...
  report =z= < 0
- =has_gradients_point=: True if this model has gradients implemented in respect
  to the point being projected
- =has_gradients_parameters=: True if this model has gradients implemented in
  respect to the intrinsic parameters
...
- this is a noncentral projection, so more infrastructure is required to make
  =unproject()= do something reasonable. Today, a CAHVORE =unproject()= will
  report a point at an arbitrary range. This model is described in:
