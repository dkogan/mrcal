...
mrcal.cameramodel() can take icam_extrinsics. To support creating and managing
a model object built from moving cameras

mrcal.show_geometry() and mrcal-show-geometry have icam_extrinsics_fromsolve and
--icam-extrinsics-from-solve. This is useful for visualizing moving cameras. If
given, instead of showing one set of camera axes per model given on the
commandline, we instead take the ONE model from the commandline, and use the
extrinsics in its optimization_inputs. The extrinsics we use are given in
--icam-extrinsics-from-solve as a ,-separated list of integers A and/or A-B
ranges. A value of -1 means "identity", useful to show cameras at the
optimization reference. A special value of "all" means "show ALL of the
extrinsics". This option is useful for moving-camera solves

I can save .pgm and .ppm images

When calibration a splined model, I use the NOMINAL fxycxy
  The spline coefficients can compensate for any fxycxy, and it makes life easier
  if the numbers used were the expected simple values

Added the mrcal-report-fov tool

New [[file:mrcal-python-api-reference.html#-compose_R][=mrcal.compose_R()=]] and =mrcal_compose_R()= functions. This is just a matrix
multiplication, but it's nice to have this explicitly available for consistency.

No more Depends:matplotlib

- C API: mrcal_cameramodel_...() to replace the old functions:
  
  - mrcal_read_cameramodel_string()
  - mrcal_read_cameramodel_file()
  - mrcal_read_cameramodel_string_into()
  - mrcal_read_cameramodel_file_into()
  - mrcal_free_cameramodel()
  - mrcal_write_cameramodel_file()

  The new functions have better names. Old functions remain for compatibility

- New mrcal_cameramodel_clone() function
...
* mrcal c api stuff should mostly be double* instead of mrcal_point3_t

* m-c-c should be able to do extrinsics-only solves better
- I need to pass in intrinsics. Currently I do that with --seed, which is bad
  because:
  - It's ,-separated which breaks tab-completion
  - I don't want to seed the extrinsics. I want to load the intrinsics and use
    the normal seeding algorithm for extrinsics
- In this case I should be able to have non-matching --lensmodel
