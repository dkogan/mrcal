...
In addition to the high errors at the edges, this plot also shows an anomalous
ring of poorly-fitting observations at the center. This could be cauesd by the
solver sacrificing accuracy at the center of the image to get a better fit in
the much larger areas further out. Clearly this lens model cannot fit everywhere
at once. Let's look at the systematic errors in another way: let's look at all
the residuals over all the observations, color-coded by their direction,
ignoring the magnitudes:
...
ring in the center is still there, however. This look like a peculiar image
processing artifact of the Sony Alpha 7 III camera: other cameras using this
lens do not exhibit this effect. A denser spline spacing maybe can model
whatever this is. We move on.
