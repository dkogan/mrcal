...
mostly reasonable. There is a pattern, however: the errors are mostly radial in
the chessboard (point to/from the center). A unmodeled chessboard flex would
cause this kind of pattern. This could cause biases later on. Let's keep going,
...
the corners of the image. We can clearly see this here in the 10th-worst image
(10th not 1st because the /really/ poor-fitting points are thrown out as
outliers):
...
/This/ is clearly a problem. The points near the corners have poor residuals,
and the entire column of points at the edge was thrown out as outliers. We note
that this was observation 47, so that we can come back to it later.
