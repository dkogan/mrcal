...
- Only the response to chessboard observation noise is taken into account. Any
  ...
  motion blur, out-of-synchronization images, unexpected chessboard shape. It is
  thus imperative that we try to minimize these issues, and mrcal provides [[file:how-to-calibrate.org::#interpreting-results][tools]]
  to detect some of these problems.
...
- Currently the uncertainty estimates can be computed only from a vanilla
  ...
  those cases. It can be done, but the sequence presented below would have to be
  expanded to do so.
...
Let's assume we're already optimized, so $\vec x = \vec x^*$ and $J^T \vec x^* = 0$, and thus
...
coordinate systems we have are floating random variables? We use the most
...
See the [[file:tour-uncertainty.org][tour of mrcal]] for a simulation validating the approach and for some
empirical results.
