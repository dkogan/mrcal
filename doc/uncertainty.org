...
calibration quality: the residual fit error. This is very inadequate however,
since we can always improve this metric by throwing away input data. And it
doesn't make sense that using less data would make a calibration /better/.

mrcal addresses this by providing [[file:how-to-calibrate.org][some tools]] to gauge how well the data fits the
model. And by providing and estimate of projection uncertainty via the
...
1. Estimate the [[file:formulation.org::#noise-model][noise in the chessboard observations]] input to the [[file:formulation.org][optimization
   routine]]
