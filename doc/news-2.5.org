...
- [[file:mrcal-python-api-reference.html#-show_distortion_off_pinhole_radial][=mrcal.show_distortion_off_pinhole_radial()=]] works with non-opencv models

- The image load/save functions no longer use [[https://freeimage.sourceforge.io/][libfreeimage]], since that library
  is no longer maintained. I'm using [[https://github.com/nothings/stb][libstb]] for image-reading and [[https://www.libpng.org/pub/png/libpng.html][libpng]] and
...
- [[file:mrcal-python-api-reference.html#-compute_chessboard_corners][=mrcal.compute_chessboard_corners()=]] has new
  =Npoint_observations_per_board_min= kwarg to set the floor for points per
  board detection. Previously this was hardcoded to 3, which was far too low.
  The new default is =2*min(board_width,board_height)=
...
- Added =mrcal_cameramodel_converter()= "converter" function that can be used
  ...
  either path strings or [[file:mrcal-python-api-reference.html#cameramodel][=mrcal.cameramodel=]] objects as =mrcal_cameramodel_VOID_t= C
...
- =mrcal_rectified_system2()= is a new flavor of =mrcal_rectified_system()= to
  ...
  still exists. It's hardcoded to use =az_edge_margin_deg=10=. [[file:mrcal-stereo.html][=mrcal-stereo=]]
...
- All the headers renamed to remove =mrcal-= from their name. Everything is
  ...
  So the =mrcal-= in the name was superfluous. Affected headers:
...
- All the headers have been renamed to remove =mrcal-= from their name. If your
  code was =#include=-ing any of
  - =mrcal-image.h=
  - =mrcal-types.h=
  - =mrcal-internal.h=

  the code must be updated to instead =#include=
  - =image.h=
  - =types.h=
  - =internal.h=
...
- The [[https://www.github.com/dkogan/mrcal/blob/master/image.h][=mrcal_image_anytype_load()=]] function uses the =mrcal_image_void_t= image
  type. The semantics are identical, but older code might need to be tweaked.
