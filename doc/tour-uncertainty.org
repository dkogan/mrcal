...
- *Model error*. We're fitting a model to data, and there are lots of potential
  reasons why no set of parameters could fit our data. The resulting errors are
  unmodeled (by definition), we cannot analytically predict their effects.
  Instead we try hard to force these errors to zero, so that we can ignore them.
  I will talk about this more in [[file:tour-cross-validation.org][the next
  section of the tour of mrcal (cross-validation diffs)]].
