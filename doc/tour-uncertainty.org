...
across the imager at y = height/2. To do this we need to write a bit of Python
code:
...
layout. Let's see. We zoom on this plot, focusing on an arbitrary region in the
middle by passing =xrange = (2500, 3500)= to the plot command:

[[file:external/figures/uncertainty/uncertainty-splined-horizontal-scan-zoomed.svg]]

We have the least confidence at these x coordintes:
...
And let's look at the knot layout in the same region:
...
The knots are roughly at these x coordinates:
...
That's pretty close. It looks like uncertainty is highest at the knots. I
haven't studied this effect, and don't have a good sense of what it means.
Hopefully one can use this to intelligently choose a good spline spacing. In any
case, this is jitter is small, and I'm not concerned about it for the time
being.
