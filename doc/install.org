...
For other distros or if you want to get the latest release of mrcal, you can use
...
#+begin_src sh
for distro (buster bullseye bookworm trixie sid) {
...
#+begin_src sh
for distro (bionic focal jammy noble) {
    perl -a -p -i -e 'if($. == 1) { $F[1] =~ s/-([0-9]+).*\)/-$1'$distro'1)/; $F[2] = "'$distro';"; $_ = join(" ",@F) . "\n"; }' debian/changelog;
...
#+begin_src sh
for distro (bullseye bookworm trixie sid) {
    perl -a -p -i -e 'if($. == 1) { $F[1] =~ s/-([0-9]+).*\)/-$1'$distro'1)/; $F[2] = "'$distro';"; $_ = join(" ",@F) . "\n"; }' debian/changelog;
