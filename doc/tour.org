...
only one representation supported: a [[file:lensmodels.org::#splined-stereographic-lens-model][/splined stereographic/ model]].
...
wide range of implied transformations, but even the simplest models have some
transform that must be compensated for.

The above vectorfield suggests that we need to move one of the cameras up and to
the left, and then we need to rotate that camera. We can automate this by adding
a critical missing step to the procedure above between steps 1 and 2:
...
#+begin_src sh :exports none :eval no-export
mrcal-show-projection-diff --radius 500 --unset key data/board/opencv8.cameramodel data/board/splined.cameramodel \
  --hardcopy ~/jpl/mrcal/doc/figures/diff-radius500-splined-opencv8.png --terminal 'pngcairo size 1024,768 transparent noenhanced crop          font ",12"'
#+end_src
...
residual distribution in the =LENSMODEL_OPENCV8= solve has visible patterns (it is
...
Let's place 4 cameras using an =LENSMODEL_OPENCV4= distortion model side by side, and let's
...
(=LENSMODEL_OPENCV4= in this case). And then we compute how the noise in those lens
...
representable by an =LENSMODEL_OPENCV4= model, then this would be be correct, but that never
