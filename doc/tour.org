...
reality. Can we quantify that? How different are the two models we computed? An
overview follows; see the [[file:differencing.org][differencing page]] for details.
...
such as =LENSMODEL_OPENCV4=, this effect is far more pronounced:
...
#+begin_src sh
mrcal-show-projection-uncertainty --vs-distance-at center opencv8.cameramodel --set 'yrange [0:0.3]'
#+end_src
#+begin_src sh :exports none :eval no-export
