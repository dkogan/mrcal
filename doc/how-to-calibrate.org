...
[[file:splined-models.org][=LENSMODEL_SPLINED_STEREOGRAPHIC=]]. These work very well, and are able to
...
an overall sense of how this should be done. A high-level process is
...
/always/ have noise in our chessboard corners, it's important for these effects
to be small. Otherwise a recalibration of the same system would produce very
different results due to new chessboard observations containing new noise.
...
uncertainty will usually be better than the uncertainty at infinity; so if we're
doing well at infinity, we'll do well at other distances also.
...
cross-validation diffs and/or the residuals to confirm that no model errors are
present. If we can confirm that, the projection uncertainties can be used as the
authoritative gauge of the quality of our calibration.
...
change, the uncertainty wouldn't either.
...
indeed exists a model error. In that case the issue was a noncentral behavior of
the lens, which wasn't fixable with today's mrcal. So we could either
...
residuals (next section)]] and reasoning. 
...
Usually I only look at the residuals if I don't trust something about the way
the data was collected: if I have little faith in the camera time-sync for
instance. Or if something isn't working properly and
...
mrcal.show_residuals_board_observation(optimization_inputs, 0,
                                       vectorscale = 100,
                                       image_directory = '~/projects/mrcal-doc-external/2022-11-05--dtla-overpass--samyang--alpha7/2-f22-infinity/images',
                                       wait = True)
#+end_src
