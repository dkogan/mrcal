...
* Capturing images
We need to get observations of a [[file:index.org::#Calibration object][calibration object]], a board containing an
...
- =--lensmodel= specifies which lens model we're using for /all/ the cameras. In
  ...
  model. Please read the [[file:index.org::#splined models configuration selection][notes]] about choosing the configuration parameters. See.
  The full set of supported lens models is described in detail [[file:index.org::#Lens models][here]].
