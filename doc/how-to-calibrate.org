...
According to the [[file:tour-choreography.org][dance study]], we want the chessboard to be as small as possible,
...
- =--lensmodel= specifies which lens model we're using for /all/ the cameras. In
  ...
  initially. And once that works well, to move to
  [[file:lensmodels.org::#splined-stereographic-lens-model][=LENSMODEL_SPLINED_STEREOGRAPHIC=]] to get better accuracy and reliable
  uncertainty reporting. This is the model of choice for all cases, but it's
  still relatively new, so starting out with something like =LENSMODEL_OPENCV8=,
  is still a good sanity check. For /very/ wide fisheye lenses,
  ...
  start there if you have an ultra-fisheye lens.
...
instead of =LENSMODEL_OPENCV8=, see the [[file:tour-initial-calibration.org::#splined-stereographic-fit][tour of mrcal]].
...
splined model (from the [[file:tour-initial-calibration.org::#splined-stereographic-fit][tour of mrcal]]):
