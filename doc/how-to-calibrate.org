...
rigid or completely flat, so my board is backed with 2.5cm of foam. This is
fairly flat and rigid. The board does warp with variations in temperature and
humidity, but this is slow, and the shape is usually stable long-enough for a
chessboard dance.
...
- =--lensmodel= specifies which lens model we're using for /all/ the cameras. In
  ...
  uncertainty reporting. For /very/ wide fisheye lenses,
...
Once again, a [[file:lensmodels.org::#splined-stereographic-lens-model][splined lens model]] resolves these biases, as seen in the [[file:tour-initial-calibration.org::#splined-stereographic-fit][tour of
mrcal]]:
