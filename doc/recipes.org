...
These kinds of solves usually work off far less data than moving-chessboard
solves, so they are more susceptible to [[file:uncertainty.org][sampling error causing high projection
uncertainty]]. The usual rule-of-thumb is to gather calibration data at multiple
ranges because it is otherwise difficult to disentangle the effects of camera
position from the effects of intrinsics. We can demonstrate this using the
[[https://www.github.com/dkogan/mrcal/blob/master/test/test-surveyed-calibration.py][=test/test-surveyed-calibration.py=]] script. Let's solve a calibration problem,
calibrating a long lens looking at 3 chessboards 10m out, with the range to the
center chessboard varying. We solve at each qualitative configuration multiple
times, and plot the mean and standard deviation of the error in the camera
forward direction. The standard deviation is an analogue of solve uncertainty,
which is not yet implemented for this kind of solve.
...
| feedgnuplot \
    ...
    --vnl     \
    --autolegend \
    --style 'mean(errz)' 'with linespoints lw 2' \
    --xlabel 'Distance to the middle chessboard (m)' \
    --ylabel 'Error and standard deviation of the error in camera z' \
    --legend 'mean(errz)'  'Mean error in camera z' \
    --legend 'stdev(errz)' 'Standard deviation in the error in camera z' \
...
[[file:external/figures/surveyed-calibration/errz.svg]]

So when all 3 chessboards sit at 10m out, we see a very high uncertainty in
camera position (and by extension, camera intrinsics). When running the standard
moving-chessboard calibration we have an [[file:tour-choreography.org::#tilt][analogous effect when we consider
tilting the chessboard to the camera]]: the chessboard tilt creates the variable
ranges required for a well-defined solve.
...
Ignoring noncentral effects very close to the lens, a camera model maps
