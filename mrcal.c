...
static double apply_noncentral( // output
                                double* dzadj_dk,
                                double* dzadj_dp,

                                // input
                                // noncentral coefficients
                                const double* k,
...
static void apply_noncentral_grad( // input, output
                                   mrcal_point3_t* du_dp,

                                   // input
                                   const double* dzadj_dp)
{
    ...
    {
        for(int j=0; j<2; j++)
            du_dp[i].xyz[j] += du_dp[i].xyz[2]*dzadj_dp[j];
        du_dp[i].xyz[2] *= dzadj_dp[2];
    }
}
                            ...
                            double segments_per_u)
{
    ...
    double mag_p =
        sqrt( p->x*p->x +
              p->y*p->y +
              zadj*zadj );
    double scale = 2.0 / (mag_p + zadj);

    mrcal_point2_t u = {.x = p->x * scale,
                        .y = p->y * scale};
    // du/dp = d/dp ( xy * scale )
    //       = pxy dscale/dp + [I; 0] scale
    // dscale/dp = d (2.0 / (mag_p + zadj))/dp =
    //           = -2/()^2 ( [0,0,1] + dmag/dp)
    //           = -2/()^2 ( [0,0,1] + 2pt/2mag)
    ...
    apply_noncentral_grad((mrcal_point3_t*)du_dp, dzadj_dp);
    ...
    for(int icam_intrinsics=0; icam_intrinsics < Ncameras_intrinsics; icam_intrinsics++)
    {
        ...
        if( problem_selections.do_optimize_intrinsics_distortions )
        {
            ...
            for(; i<Ndistortions; i++)
                b[i_state++] = intrinsics[Ncore + i] / SCALE_DISTORTION_NONCENTRAL;
        }
                                         ...
                                         int Ncameras_intrinsics )
{
    ...
    for(int icam_intrinsics=0; icam_intrinsics < Ncameras_intrinsics; icam_intrinsics++)
    {
        ...
        if( problem_selections.do_optimize_intrinsics_distortions )
        {
            ...
            for(; i<Ndistortions; i++)
                b[i_state++] /= SCALE_DISTORTION_NONCENTRAL;
        }
    }
    return i_state;
}
    ...
    for(int icam_intrinsics=0; icam_intrinsics < Ncameras_intrinsics; icam_intrinsics++)
    {
        ...
        if( problem_selections.do_optimize_intrinsics_distortions )
        {
            ...
            for(; i<Nintrinsics-Ncore; i++)
                intrinsics[icam_intrinsics*intrinsics_stride + Ncore + i] = b[i_state++] * SCALE_DISTORTION_NONCENTRAL;
        }
    }
    return i_state;
}
