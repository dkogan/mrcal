...
def show_stereographic_focal_surface(model,
                                     xy,
                                     gridn_x          = 60,
                                     gridn_y          = 40,
    ...
    I can interpret any projection function as stereographic projection that
    uses a focal length that varies across the imager. The shape of this
    variable focal-length surface varies depending on the model. The
    LENSMODEL_SPLINED_STEREOGRAPHIC_... models work by literally encoding this
    surface with splines. This surface can be computed and visualized for any
    model, however.

    This function samples the imager. For each imager point I unproject to get
    the observation vector. Then I compute the stereographic projection, and
    report the scaling difference from the original imager point.
    ...
    if 'title' not in kwargs:
        ...
        title = f"{xy} focal-length surface for {lensmodel}"
        if extratitle is not None:
            title += ": " + extratitle
        kwargs['title'] = title
    ...
    if mrcal.getLensModelMeta(lensmodel)['has_core']:
        ...
        try:
            m = re.match('LENSMODEL_SPLINED_STEREOGRAPHIC_.*_([0-9]+)_([0-9]+)$',
                         lensmodel)
            cxy = np.array((int(m.group(1)), int(m.group(2))))
        except:
            raise Exception(f"lensmodel {lensmodel} has no parseable cxy")

    # shape: Nwidth,Nheight,2
    q  = np.ascontiguousarray(nps.reorder(nps.cat(*np.meshgrid(np.linspace(0,W-1,gridn_x),
                                                               np.linspace(0,H-1,gridn_y))),
                                          -1, -2, -3),
                              dtype = float)

    v = mrcal.unproject(q, lensmodel, intrinsics_data)
    u = mrcal.projectStereographic(v, 1,1,0,0)
